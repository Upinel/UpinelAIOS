...
def main():
    ...
    try:
        ...
        open(os.path.join(root, "good--model-a", "weights-Q4_0.gguf"), "w").close()
        os.makedirs(os.path.join(root, "good--model-b"))
        open(os.path.join(root, "good--model-b", "weights-Q4_K_M.gguf"), "w").close()
        ...
        # A directory holding only a projector is not a model.
        os.makedirs(os.path.join(root, "proj--only"))
        open(os.path.join(root, "proj--only", "mmproj-BF16.gguf"), "w").close()
