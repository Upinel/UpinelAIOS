...
| model | decode | prefill | MTP depth |
|---|---:|---:|---:|
| **`mlx-q-35ba3b`** — Qwen 3.6 35B-A3B, 22 GB | **~98 t/s** | ~1,520 t/s | 1 |
| `mlx-q-27b-4bit` — Qwen 3.8 27B dense, 19 GB | ~31 t/s | ~366 t/s | 2 |
| `mlx-q-9b` — Qwen 3.8 9B, 5 GB | ~90 t/s | ~1,390 t/s | — |

**Re-measured at 8k context**, cold, after the benchmark was fixed to stop
reusing one prompt and measuring its own prefix cache. The figures this table
used to carry - 79.4 / 34.7 / 65.1 decode and a 32 t/s prefill - predate that
fix; the prefill number especially was about 47x low. Individual runs still vary
widely, because MTP commits a variable number of tokens per forward pass
depending on how predictable the continuation is.

The old alias names (`moe`, `4bit`, `9b`) still work and still resolve to the
same weights - see `bench/verify-legacy-aliases.py`.
...
Or switch at launch without editing anything:

```bash
./restart.sh --model mlx-q-27b-4bit     # switch model and engine in one step
```

Everything else on this list is worth single-digit percent by comparison.

### Gemma 4 - why it is not on the MLX side
