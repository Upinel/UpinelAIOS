...
#
# Engine-agnostic: the service runs ./start.sh, which picks llama.cpp or MTPLX
# from the model in env.conf. Changing MODEL changes which engine the service
# starts, with no change here.
#
...
<dict>
  ...
  <!-- The process needs the same PATH a login shell has, because the runtimes
       in /opt/homebrew/bin. -->
